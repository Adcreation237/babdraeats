...
class UsersForm extends StatefulWidget {
  final String email;
  final String mdp;
  const UsersForm({Key? key, required this.email, required this.mdp})
      : super(key: key);
...
class _UsersFormState extends State<UsersForm> {
  ...
  Future showNotification(String email) async {
    ...
    await localNotification!.show(
        0,
        "Babdra Eats",
        "Hello ! $email. Bienvenue dans notre communauté",
                      ...
                      onPressed: () async {
                        if (formKey.currentState!.validate()) {
                          await authService.createUserWithEmailAndPassword(
                              widget.email.toString(), widget.mdp.toString());
                          showNotification(widget.email.toString());
                          manipulateUsers.addusers(
                              authService.uid,
                              widget.email,
                              name.text,
                              widget.mdp,
                          ...
                          Navigator.pushReplacement(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => BottomBar(pages: 0)));
                        }
                      },
                      height: 50,
